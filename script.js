...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('static/sw.js').then(() => console.log("PWA Ready")).catch(err => console.log("PWA Error", err));
    }
...
async function handleFileUpload(e) {
    ...
    status.innerHTML = '⚡ Procesando en tu celular...';
    ...
    reader.onload = async function() {
        ...
        try {
            ...
            for (let i = 1; i <= pdf.numPages; i++) {
                ...
                // Better heuristic: look for lines or chunks that look like time ranges
                // Regex for formats like "08:00 - 09:30" or "8:00 to 10:00"
                const timeRegex = /(\d{1,2}[:.]\d{2})\s*[-–to ]+\s*(\d{1,2}[:.]\d{2})/g;
                let match;
                
                // Strategy: split by text chunks and try to find time nearby
                textContent.items.forEach((item, index) => {
                    const timeMatch = item.str.match(/(\d{1,2}[:.]\d{2})/);
                    if (timeMatch) {
                        // Potential time found, check next few items for subject
                        let startTime = timeMatch[0].replace('.', ':');
                        let subject = "";
                        // Look ahead for name
                        for(let j=1; j<4; j++) {
                            if (textContent.items[index+j] && textContent.items[index+j].str.trim().length > 3) {
                                subject = textContent.items[index+j].str.trim();
                                break;
                            }
                        }
                        if (subject && !subject.match(/\d/)) { // Heuristic: subject shouldn't be mostly numbers
                            extractedData.push({ subject, start: startTime });
                        }
                    }
                });
            }
            ...
            if (extractedData.length > 0) {
                pendingPdfData = extractedData;
                document.getElementById('pdf-group-name').value = file.name.replace('.pdf', '');
                openModal('pdf-name-modal');
                status.innerHTML = '';
            } else {
                status.innerHTML = '⚠️ No encontré horarios legibles. Intenta con otro PDF.';
            }

        } catch (err) {
            console.error(err);
            status.innerHTML = '❌ Error al leer el PDF';
        }
    };
...
function saveManualAlarm() {
    ...
    if(!sub || !time) return alert("Faltan datos");
